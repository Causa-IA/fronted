<script>
  ...
  try {
    const response = await fetch(`${API_URL}/consultas/get_consultas_estudiante/{id_estudiante}`);
...
<div class="container fade-in">
  ...
  <div class="search-box">
    <input
      ...
      bind:value={cedula}
      on:keydown={(e) => { if (e.key === "Enter") buscarConsultas(); }}
    />
    <button class="btn-buscar" on:click={buscarConsultas}>Buscar</button>
  </div>
      ...
      <tbody>
        {#each consultas as consulta}
          <tr on:click={() => verConsulta(consulta.id_consulta)}>
            <td>{consulta.nombre_estudiante || `${consulta.primer_nombre ?? ''} ${consulta.primer_apellido ?? ''}`}</td>
  ...
  <!-- ← DENTRO del container, debajo de la tabla -->
...
<style>
  ...
  /* Botón volver — dentro del container, al final */
  .btn-volver {
    display: block;
    margin: 24px auto 0;
    padding: 12px 28px;
