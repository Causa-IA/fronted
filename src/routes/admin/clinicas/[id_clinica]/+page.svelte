<script>
  ...
  const id_clinica = $page.params.id_clinica;
  ...
  async function cargarClinica() {
    try {
      const res = await fetch(`${API}/clinicas/get_clinica/${id_clinica}`);
      ...
      error = 'Error al cargar la clínica';
    }
  }
  ...
  async function guardar() {
    guardando = true;
    try {
      const res = await fetch(`${API}/clinicas/${form.id_clinica}`, {
        method: 'PUT',
  ...
  async function eliminar() {
    const confirmar = confirm('¿Estás seguro de que deseas eliminar esta clínica?');
    if (!confirmar) return;
...
<div class="page">
  ...
  {:else}
    <div class="top">
      <div>
        <button class="btn-volver" onclick={() => goto('/admin/clinicas')}>← Volver</button>
        <h2>{form.nombre}</h2>
        <p class="sub">{form.ubicacion}</p>
      </div>
      <div class="acciones">
    ...
    <div class="card">
      <p class="seccion-titulo">Información de la clínica</p>
